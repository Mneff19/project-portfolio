import rlHandler from "./rlHandler";

// Handles setting the depth and calculating the sequence
export default class Sequence {
    depth: number = 0; // How many numbers in the sequence to calculate
    question: string = '\nHow deep into the sequence would you like to calculate? (1-100)\n> '; // Question shown to the user
    rlh: rlHandler = new rlHandler(this.question, this.handleDepthSetting.bind(this)); // Handles getting user input for depth
    calculatingSequence = false; // Prevents calculations from running twice (rl caused it to run multiple times without this)
     ...
     */
    handleDepthSetting(response: number) {
        if (response > 0) {
     ...
     */
    _calculateSequenceRecursive(n0: number, n1: number, runningDepth: number) {
